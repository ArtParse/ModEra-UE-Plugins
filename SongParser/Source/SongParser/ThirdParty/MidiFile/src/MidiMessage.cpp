...
//#include "RhythmGame/ThirdParty/MidiFile/include/MidiMessage.h"
#include "../ThirdParty/MidiFile/include/MidiMessage.h"
#include <cmath>
#include <iostream>
#include <iomanip>
#include <iterator>
...
void MidiMessage::makeMts9_TemperamentByCentsDeviationFromET (std::vector<double>& mapping, int referencePitchClass, int channelMask) {
	...
	for (int i=0; i<(int)mapping.size(); i++) {
		...
		uchar LSB2 = intval & 0x7f; // Изменено имя переменной
		uchar MSB2 = (intval >> 7) & 0x7f; // Изменено имя переменной
		data.push_back(MSB2);
		data.push_back(LSB2);
